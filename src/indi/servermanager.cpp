...
#endif
        serverProcess->start(Options::iNDIServerPath(), args);
...
void ServerManager::insertEnvironmentPath(QProcessEnvironment *env, QString variable, QString relativePath)
{
    ...
    if (QFileInfo::exists(environmentPath) && Options::iNDIDriversDefault())
